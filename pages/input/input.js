...
Page({
  ...
  async _add(obj,type,who,direct=false){
    ...
    var url = `${app.globalData.baseURL}/first_ask?idtext=&input_text=${value}`
    console.log("url:",url)
    res1 = await util.request({
       url:url
     })
    ...
    else if (Object.keys(res1.data)[0] == "text0") {
      this.data.items.push({ msg: res1.data.text0, type: "oth", dateTime: (new Date()).toISOString() })
    }
    // 聊天
    else{
  ...
  async _onQrcodeTap(){
    ...
    util.scanCode().then((res) => {
      this.setData({ value: "识别码:" + res.result })
      console.log(11111111111, res.result)
      this._add("识别码:" + res.result, "self")

      // util.request({ url: `${app.globalData.baseURL}/input_id?idtext=${value}`
      // }).then((res10)=>
